...
// CODE FROM HERE: https://github.com/mattdean1/cuda/tree/master
...
int PREFIX_THREADS_PER_BLOCK = 512;
int ELEMENTS_PER_BLOCK = PREFIX_THREADS_PER_BLOCK * 2;
...
void scanLargeEvenDeviceArray(int *d_out, int *d_in, int length, bool bcao)
{
    ...
    if (bcao)
    {
        prescan_large<<<blocks, PREFIX_THREADS_PER_BLOCK, 2 * sharedMemArraySize>>>(d_out, d_in, ELEMENTS_PER_BLOCK, d_sums);
    }
    else
    {
        prescan_large_unoptimized<<<blocks, PREFIX_THREADS_PER_BLOCK, 2 * sharedMemArraySize>>>(d_out, d_in, ELEMENTS_PER_BLOCK, d_sums);
    }

    const int sumsArrThreadsNeeded = (blocks + 1) / 2;
    if (sumsArrThreadsNeeded > PREFIX_THREADS_PER_BLOCK)
    {
